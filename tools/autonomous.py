...
class AutonomousExecutionTools:
    ...
    async def autonomous_filesystem_full_v2(
        ...
        """
        Full autonomous execution with filesystem MCP using /v1/responses API (OPTIMIZED).

        This is the optimized version that uses LM Studio's stateful /v1/responses API
        instead of /v1/chat/completions. Benefits:
        - ✅ Constant token usage (no message history growth)
        - ✅ 98% fewer tokens vs v1 at round 100 (3K vs 127K!)
        - ✅ Simpler code (server maintains conversation state)
        - ✅ Better scalability (can run many more rounds)
        ...
        try:
            ...
            # Determine working directory/directories
            if working_directory:
                working_dirs = validate_working_directory(working_directory)
            else:
                default_dir = os.getenv("WORKING_DIR") or os.getcwd()
            ...
            import traceback
            return f"Error during filesystem execution (v2): {str(e)}\n\n{traceback.format_exc()}"
    ...
    async def autonomous_memory_full_v2(
        ...
        """
        Full autonomous execution with memory MCP using /v1/responses API (OPTIMIZED).

        This is the optimized version that uses LM Studio's stateful /v1/responses API
        instead of /v1/chat/completions. Benefits:
        - ✅ Constant token usage (no message history growth)
        - ✅ 70-80% fewer tokens vs v1 at round 100
        - ✅ Simpler code (server maintains conversation state)
        - ✅ Better scalability (can run many more rounds)
        ...
        try:
            ...
            import traceback
            return f"Error during memory execution (v2): {str(e)}\n\n{traceback.format_exc()}"
    ...
    async def autonomous_fetch_full_v2(
        self,
        task: str,
        max_rounds: int = 100,
        max_tokens: Union[int, str] = "auto"
        ...
        """
        Full autonomous execution with fetch MCP using /v1/responses API (OPTIMIZED).
        ...
        try:
            ...
            import traceback
            return f"Error during fetch execution (v2): {str(e)}\n\n{traceback.format_exc()}"
    ...
    async def autonomous_github_full_v2(
        ...
        """
        Full autonomous execution with GitHub MCP using /v1/responses API (OPTIMIZED).

        This is the optimized version that uses LM Studio's stateful /v1/responses API
        instead of /v1/chat/completions. Benefits:
        - ✅ Constant token usage (no message history growth)
        - ✅ 94% fewer tokens vs v1 at round 100 (7.5K vs 130K!)
        - ✅ Simpler code (server maintains conversation state)
        - ✅ Better scalability (can run many more rounds)
        ...
        try:
            ...
            import traceback
            return f"Error during GitHub execution (v2): {str(e)}\n\n{traceback.format_exc()}"
...
def register_autonomous_tools(mcp, llm_client: Optional[LLMClient] = None):
    ...
    async def autonomous_filesystem_full_v2(
        ...
        Examples:
            ...
            # Different project directory
            autonomous_filesystem_full_v2("Read README.md", working_directory="/path/to/other/project")
        ...
        Note: This v2 version uses the optimized /v1/responses API which provides
        massive token savings (98% at round 100) compared to v1 (which uses /v1/chat/completions).
        """
        return await tools.autonomous_filesystem_full_v2(task, working_directory, max_rounds, max_tokens)
    ...
    async def autonomous_memory_full_v2(
        ...
        Note: This v2 version uses the optimized /v1/responses API which provides
        massive token savings compared to v1 (which uses /v1/chat/completions).
        """
        return await tools.autonomous_memory_full_v2(task, max_rounds, max_tokens)
    ...
    async def autonomous_fetch_full_v2(
        ...
        Note: This v2 version uses the optimized /v1/responses API which provides
        token savings compared to v1 (which uses /v1/chat/completions).
        """
        return await tools.autonomous_fetch_full_v2(task, max_rounds, max_tokens)
    ...
    async def autonomous_github_full_v2(
        ...
        Note: This v2 version uses the optimized /v1/responses API which provides
        massive token savings (94% at round 100) compared to v1 (which uses /v1/chat/completions).
        """
        return await tools.autonomous_github_full_v2(task, github_token, max_rounds, max_tokens)
