...
class TestJITGuardConsolidation:
    ...
    def test_single_call_site_for_ensure_model_loaded_with_verification(self):
        """ensure_model_loaded_with_verification should appear exactly ONCE (inside jit_loader)."""
        source_path = os.path.join(os.path.dirname(__file__), "..", "llm", "jit_loader.py")
        with open(source_path) as f:
            source = f.read()
        ...
        assert count == 1, (
            ...
            f"(inside jit_loader.ensure_model_loaded), found {count}"
        )
