...
class TestVersionConsistency:
    ...
    def test_constants_version_matches_setup_version(self):
        """VERSION in config/constants.py must match version in setup.py."""
        # Read config/constants.py
        ...
        # Extract VERSION from constants.py (must be at start of line)
        const_match = re.search(r'^VERSION\s*=\s*["\']([^"\']+)["\']', constants_content, re.MULTILINE)
        assert const_match, "Could not find VERSION in config/constants.py"
    ...
    def test_python_requires_matches_constants(self):
        """python_requires in setup.py must match MIN_PYTHON_VERSION in constants.py."""
        with open("config/constants.py", "r") as f:
            constants_content = f.read()
        ...
        assert const_match, "Could not find MIN_PYTHON_VERSION in config/constants.py"
